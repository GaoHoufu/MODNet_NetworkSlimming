...
from src.models.modnet import LRBranch,HRBranch,FusionBranch,IBNorm
# new add
...
# ----------------------------------------------------------------------------------
# 半精度训练
# blurer = GaussianBlurLayer(1, 3).cuda().half()
blurer = GaussianBlurLayer(1, 3).cuda()

# 模型剪枝部分
# sparsity-induced�ͷ���ĸ��Ӵ��ݶ��½�
# def updateBN(model):
...
# def updateBN(model):
    ...
    #                 m1.weight.grad.data.add_(1e-4*torch.sign(m1.weight.data))  # L1
    #     elif isinstance(m, IBNorm):
    #         for m2 in m.modules():
    #             if isinstance(m2, nn.BatchNorm2d):
    #                 m2.weight.grad.data.add_(1e-4*torch.sign(m2.weight.data))
        ...
        if isinstance(m, MobileNetV2):
            for m1 in m.modules():
                if isinstance(m1, InvertedResidual):
                    if len(m1.conv)>5:
                        m1.conv[1].weight.grad.data.add_(1e-4*torch.sign(m1.conv[1].weight.data))
                        m1.conv[4].weight.grad.data.add_(1e-4*torch.sign(m1.conv[4].weight.data))
        elif isinstance(m, IBNorm):
            for m1 in m.modules():
                if isinstance(m1, nn.BatchNorm2d):
                    m1.weight.grad.data.add_(1e-4*torch.sign(m1.weight.data))
def supervised_training_iter(
    modnet, optimizer, image, trimap, gt_matte,idx,
    semantic_scale=10.0, detail_scale=10.0, matte_scale=1.0, compactor_mask_dict=None):
    ...
    Example:
        ...
        dataloader = CREATE_YOUR_DATALOADER(bs)     # NOTE: please finish this function #���ݼ�����

        for epoch in range(0, epochs):
            for idx, (image, trimap, gt_matte) in enumerate(dataloader):
                semantic_loss, detail_loss, matte_loss = \ #���ӻ�
    ...
    # 半精度训练
    # gt_semantic= gt_semantic.type(torch.FloatTensor).cuda()
    
    gt_semantic = blurer(gt_semantic)
    # 半精度训练
    # gt_semantic= gt_semantic.type(torch.HalfTensor).cuda()
