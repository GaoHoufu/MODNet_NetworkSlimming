...
pruned_model_save = "./src/convert_new/second_pruning_24_0.0167_{}_{}.pth".format(ratio_mobilenetv2,ratio_modnet_other)
pruned_model_save_v2 = "./src/convert_new_v2/second_pruning_24_0.0167_v2_{}.pth".format(ratio_mobilenetv2)
...
model_path = "./model_save/original_model_YOLO_4w_prue_MBV2_prue/new_trimap_0024_lr0.0001.pth"
...
# ???mobilenetv2????????????????��???�???mobilenetv2???????��???�?????v2???????????????��????
# ---------------MobileNetV2???????----------------
# ????????bn???????total???????mobilenetv2???????��?bn???????
total = 0
highest_thre = []
            ...
            if isinstance(m1, InvertedResidual):
                ...
                if len(m1.conv) > 5:  
                    ...
                    if count in can_cha:
                        ...
                        weight_combine = (can_cha_last[-1]+weight_copy)/2
                        mask1 = weight_combine.abs().gt(thre).float().cuda()
                        mask2 = weight_copy.abs().gt(thre).float().cuda()
                        mask = (mask1+mask2).abs().gt(1).float().cuda()
                        mask_x = cfg_mask[-3]
                        if int(torch.sum(mask)) < int(torch.sum(mask_x)):
                            choose_first = int(torch.sum(mask_x))
                            data_mask, idx_mask = torch.sort(weight_combine.abs())
                            mask[idx_mask[-choose_first:]] = 1
                            cfg.append(int(torch.sum(mask)))
                            cfg_mask.append(mask.clone())
                            ...
                            if count ==10:
                                data_x_x_x, idx_x_x_x = torch.sort(((can_cha_last[-3]+can_cha_last[-2])/2).abs())
                                cfg_mask[-9][idx_x_x_x[-choose_first:]] = 1
                                cfg[-9] = int(torch.sum(cfg_mask[-9]))

                                data_x_x, idx_x_x = torch.sort(((can_cha_last[-2]+can_cha_last[-1])/2).abs())
                                cfg_mask[-6][idx_x_x[-choose_first:]] = 1
                            ...
                            data_x, idx_x = torch.sort(weight_combine.abs())
                            mask_x[idx_x[-choose_first:]] = 1
                            cfg_mask[-3] = mask_x
